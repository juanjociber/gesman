<?php
	...
	$objPHPExcel->setActiveSheetIndex(0)
		...
		->setCellValue('Q1', 'TECNICO')
		->setCellValue('R1', 'ING_TRABAJO')
		->setCellValue('S1', 'ING_REFRIGERIO')
		->setCellValue('T1', 'SAL_REFRIGERIO')
		->setCellValue('U1', 'SAL_TRABAJO')
	...
	try{
		...
		if(FnDiferenciaFechas($_GET['fechainicial'], $_GET['fechafinal'])>120){throw new Exception("No podemos generar un reporte mayor a 120 días.");}
		...
		$datos=array();
		$datos=FnReporteOrdenesTareos($conmy, $search);
		...
		if(count($datos)){
			...
			foreach($datos as $key=>$valor){
				...
				$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue('Q'.$i, $valor['tecnico'])
					->setCellValue('R'.$i, empty($valor['ing_trabajo'])?null:PHPExcel_Shared_Date::PHPToExcel(date("d/m/Y H:i:s", strtotime($valor['ing_trabajo']))))
					->setCellValue('S'.$i, empty($valor['ing_refrigerio'])?null:PHPExcel_Shared_Date::PHPToExcel(date("d/m/Y H:i:s", strtotime($valor['ing_refrigerio']))))
					->setCellValue('T'.$i, empty($valor['sal_refrigerio'])?null:PHPExcel_Shared_Date::PHPToExcel(date("d/m/Y H:i:s", strtotime($valor['sal_refrigerio']))))
					->setCellValue('U'.$i, empty($valor['sal_trabajo'])?null:PHPExcel_Shared_Date::PHPToExcel(date("d/m/Y H:i:s", strtotime($valor['sal_trabajo']))))
					->setCellValue('V'.$i, $valor['minutos']);
				$i+=1;
			};
			$objPHPExcel->getActiveSheet()->getStyle('G2:G'.$i)->getNumberFormat()->setFormatCode("dd/mm/yyyy");
			$objPHPExcel->getActiveSheet()->getStyle('R2:R'.$i)->getNumberFormat()->setFormatCode("dd/mm/yyyy hh:mm:ss");
			$objPHPExcel->getActiveSheet()->getStyle('S2:S'.$i)->getNumberFormat()->setFormatCode("dd/mm/yyyy hh:mm:ss");
			$objPHPExcel->getActiveSheet()->getStyle('T2:T'.$i)->getNumberFormat()->setFormatCode("dd/mm/yyyy hh:mm:ss");
			$objPHPExcel->getActiveSheet()->getStyle('U2:U'.$i)->getNumberFormat()->setFormatCode("dd/mm/yyyy hh:mm:ss");
		}else{
	...
	// Renombrar la hoja
	$objPHPExcel->getActiveSheet()->setTitle('reporte_ordenes_tareos_120_dias');
	...
	header('Content-Disposition: attachment;filename="reporte_ordenes_tareos_120_dias_'.date("YmdHis").'.xlsx"');
